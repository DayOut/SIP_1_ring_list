...
public:
    ...
    void sort_now_elem() {
        ...
        if (head) 
        {
            ...
            {
                pos = tail; // рассматриваем элементы с головы (просто смотрим сразу next елемент, вот с хвоста и начали)
                while (pos->Next->Inf < elem->Inf)  // делаем до тех пор пока текущий элемент больше курсора (будем вставлять сразу за найденным элементом)
                {
                    pos = pos->Next; // переход к следующему
                    if (pos->Next == head) // если уже прошли цикл и не нашли (значит надо вставлять после хвоста)
                    {
                        break;
                    }
                }
